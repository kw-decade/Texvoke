...
- **从机制层面限制工具调用的上游**。如果供应商在系统层做了约束（特定
  错误码、政策声明、或输出侧的过滤审核），说明这个上游的产品形态就不打算
  支持工具调用。
- **被中间层捂住嘴的上游**。如果上游网站会用代码过滤或改写模型的原始输出，
  调用信号在到达本框架之前就没了。判据一句话：模型的原始文字能不能完整流到你手里。
- **模型选错工具或填错参数**。协议层保证格式合法，保证不了语义正确。
- **上游自己的排队与 prefill 延迟**。流式出口已是真流式（上游一出字就转发，
  实测真实上游 500 字生成的首字延迟 22.77s → 7.41s），但剩下的 7 秒是上游
  在排队和 prefill，本项目无从缩短。上游适配器未实现 `upstream.StreamWriter`
  时自动回落伪流式——行为正确，只是没有首字延迟收益。
